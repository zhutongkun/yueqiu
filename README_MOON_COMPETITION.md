...
当前分支已在目标 Jetson Orin Nano 的 `/home/ubuntu/ros_ws` 完成 Moon CPU 模型原生加载、ROS 预加载服务、无运动完整 launch、148 项自动测试和 `catkin build competition -j4 -l4`。真实比赛卡片、完整导航、机械臂、坡面、返航和离线播报仍必须在比赛场地实机确认。详见 `docs/moon_competition_test_report.md`。
...
起步的原始 3 秒前进和 3 秒原地转向保持不变。原地转向之后、两次放置之前以及离开放置区前往第二次夹取时，`move_base` 先经过负 Y 侧的可配置斜坡绕行点。斜坡没有进入二维占据地图，因此主程序同时订阅融合姿态 `/imu`：它以节点启动时的重力方向作为安装基准，检测到相对倾角超过阈值时只取消当前导航目标，沿最新底盘速度的反方向退出，恢复水平后重新发送目标，不把整场任务置为 `STOPPED`。绕行点和恢复速度均在 `moon_competition.yaml` 中配置；完整运动仍必须现场看护验证。
