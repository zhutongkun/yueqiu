...
# Runtime dependency installation is intentionally disabled for Jetson safety.
import ultralytics
...
AUTOINSTALL = False  # Runtime package installation is forbidden on the robot.
...
def check_requirements(requirements=ROOT / "requirements.txt", exclude=(), install=False, cmds="", **kwargs):
    """Check requirements without mutating the deployed Jetson environment."""
    if isinstance(requirements, Path) and sys.version_info < (3, 9):
        exclude = (*exclude, "urllib3")
    return check_requirements_ultralytics(requirements, exclude=exclude, install=False, cmds=cmds, **kwargs)
