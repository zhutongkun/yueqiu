    ...
    def color_track(self):
        while self.running:
            ...
            if self.center is not None and self.start:
                    #print(distance)
                ################
                if self.center.y > 430: 
                    self.center.y = 430
                elif self.center.y < 60:
                    self.center.y = 60
                
                #self.pid_d.SetPoint = 150
                if abs(self.center.y - 100) < 10:
                    self.center.y = 100
                self.pid_d.update(self.center.y - 100)  # 更新pid(update pid)
                self.linear_x = -misc.set_range(self.pid_d.output, -0.3, 0.3)
                
                if abs(self.center.x - 320) < 10:
                    self.center.x = 320
                self.pid_angular.update(self.center.x - 320)  # 更新pid(update pid)
                twist.linear.x = self.linear_x
                twist.angular.z = misc.set_range(self.pid_angular.output, -0.5, 0.5)
                self.mecanum_pub.publish(twist)
            else:
                rospy.sleep(0.01)
            self.mecanum_pub.publish(twist)
...
if __name__ == "__main__":
    ColorTrackNode('color_track_mecanum')
