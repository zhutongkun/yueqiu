...
class TrackAndGrabNode():
    ...
    def start_function(self):
        ...
        self.running = True
        # 设置需要夹取的策略
        mode = rospy.get_param('~detect_mode','detect_tree_style111')
    ...
    def distance_judge_pick(self,color_name):
        _, _, rgb_image, _ = self.img_pre_process()
        h, w = rgb_image.shape[:2]
        img = cv2.resize(rgb_image, (int(w/2), int(h/2)))
        ...
        while not rospy.is_shutdown() and self.running:
            if self.running:
                ...
                if self.mode == 'detect_tree_style':  # 检测果树类型
                    ...
                    tree_style = self.get_fruit_tree_feature('tree_style')
                    if tree_style == 'tree_one':
                        tree_left_fruit_color = 'red'
                    elif tree_style == 'tree_two':
                        tree_left_fruit_color = 'green'
                    else:
                        tree_left_fruit_color = None
                        continue
                    echo_str(('----- tree_left_fruit_color -----',tree_left_fruit_color),EchoColor.YELLOW)
                    print('----- tree_left_fruit_color -----',tree_left_fruit_color)
                    if tree_left_fruit_color == self.target_color:
                ...
                if self.mode == 'pick_mode_one':  
                    print('---------- pick_mode_one -----------')
                    if self.next_picking_strategy == 'same_point':
                        if self.distance_judge_pick(self.target_color) == False:
                            self.mode == 'pick_mode_one'
                            self.arm_move_flag = 0
                            continue
                        else:
