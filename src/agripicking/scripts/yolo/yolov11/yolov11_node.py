...
class Yolov11Node:
    def __init__(self, name):
        ...
        self.yolov11 = yolo11_det_trt.YoLo11TRT(os.path.join(MODE_PATH, engine),os.path.join(MODE_PATH, lib),categories)
        ...
        rospy.Service('/yolo11/start', Trigger, self.start_srv_callback) 
        rospy.Service('/yolo11/stop', Trigger, self.stop_srv_callback)  
    ...
    def start_srv_callback(self, msg):
        rospy.loginfo("start yolo11 detect")
    ...
    def stop_srv_callback(self, msg):
        rospy.loginfo('stop yolo11 detect')
        self.running = False
        self.start = False
    ...
    def image_proc(self):
        while self.running:
            ...
            try:
                if self.start:
                    # print('yolo11 running')
                    objects_info = []
                    h, w = image.shape[:2]
                    boxes, scores, classid = self.yolov11.infer_video(image)
                    for box, cls_conf, cls_id in zip(boxes, scores, classid):
                        color = yolo11_det_trt.colors(cls_id, True)
            ...
            self.result_image_pub.publish(common.cv2_image2ros(result_image, frame_id='yolo11'))
        self.yolo11.destroy() 
        rospy.signal_shutdown('shutdown')

if __name__ == "__main__":
    node = Yolov11Node('yolo11')
