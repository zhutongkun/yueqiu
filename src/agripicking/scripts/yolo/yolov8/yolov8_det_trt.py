...
def get_video_path_batches(batch_size, frame):
    ret = []
    batch = []
    for root, dirs, files in os.walk(img_dir):
        for name in files:
...
class YoLov8TRT(object):
    ...
    def __init__(self, engine_file_path, plugin_library,categories):

        if len(plugin_library) > 2:
    ...
    def infer_video(self, raw_image_generator):
        threading.Thread.__init__(self)
        # Make self the active context, pushing it on top of the context stack.
        self.ctx.push()
        # Restore
...
def defult_main(engine_file_path,PLUGIN_LIBRARY,categories):
    # a YoLov8TRT instance
    yolov8_wrapper = YoLov8TRT(engine_file_path,PLUGIN_LIBRARY,categories)
    try:
...
def myself_main(engine_file_path,PLUGIN_LIBRARY,categories):
    yolov8_wrapper = YoLov8TRT(engine_file_path,PLUGIN_LIBRARY,categories)
    classes = ['red_fruit','green_fruit','red_tag','green_tag']
    try:
        ...
        while True:
            ret, frame = cap.read()
            if ret:
                boxes, scores, classid = yolov8_wrapper.infer_video(frame)
...
if __name__ == "__main__":
    ...
    PLUGIN_LIBRARY = MODE_PATH + "/libmyplugins.so"
    engine_file_path = MODE_PATH + "/best_agripicking.engine"
    ...
    os.makedirs('output/')
    myself_main(engine_file_path,PLUGIN_LIBRARY,categories)